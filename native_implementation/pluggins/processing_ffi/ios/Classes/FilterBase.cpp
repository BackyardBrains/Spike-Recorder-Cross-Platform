...
//
        void FilterBase::filter(int16_t *data, int32_t numFrames, bool flush) {
            ...
            // if (!tempFloatBuffer) return;  // Memory allocation failed
...
//
        void FilterBase::filterContiguousData(double *data, int32_t numFrames) {
            ...
            if (!tInputBuffer || !tOutputBuffer) {
                if (tInputBuffer) free(tInputBuffer);
                if (tOutputBuffer) free(tOutputBuffer);
        ...
        void FilterBase::intermediateVariables(double Fc, double Q) {
            if (samplingRate <= 0.0 || Q <= 0.0 || Fc < 0.0) {
                // Invalid parameters, set safe defaults
                omega = 0.0;
                omegaS = 0.0;
                omegaC = 1.0;
                alpha = 0.0;
