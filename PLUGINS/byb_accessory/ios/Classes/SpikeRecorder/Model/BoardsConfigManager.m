...
-(int) loadConfig
{
    ...
    if (_fileHandle == nil)
    {
        ...
        if (_fileHandle == nil) {
            ...
            NSString *bundlePath = [pluginBundle pathForResource:@"byb_accessory_assets" ofType:@"bundle"];
            if (bundlePath.length > 0) {
                NSBundle *assetsBundle = [NSBundle bundleWithPath:bundlePath];
                filePath = [assetsBundle pathForResource:@"board-config" ofType:@"json"];
                _fileHandle = [NSFileHandle fileHandleForReadingAtPath:filePath];
            }
        }
    }
