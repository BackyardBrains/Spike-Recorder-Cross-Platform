...
- (id)init
{
    ...
    BOOL appDelegateSupportsReinitSelector =
        [appDelegate respondsToSelector:@selector(shouldReinitializeAudio)] &&
        [appDelegate respondsToSelector:@selector(setShouldReinitializeAudio:)];
    //if ((self = [super init]) && ![appDelegate launchingFromLocked])
    if (self || (self = [super init]))
    {
        NSLog(@"Create local audio devices");
        bool reinitializeMFi = false;
        if(appDelegateSupportsReinitSelector &&
           ((BOOL (*)(id, SEL))objc_msgSend)(appDelegate, @selector(shouldReinitializeAudio)))
        {
        ...
        if([audioManager shouldReinitializeAudio])
        {
            ...
            if (appDelegateSupportsReinitSelector) {
                ((void (*)(id, SEL, BOOL))objc_msgSend)(appDelegate, @selector(setShouldReinitializeAudio:), YES);
            }
        }
        else
        {
            if (appDelegateSupportsReinitSelector) {
                ((void (*)(id, SEL, BOOL))objc_msgSend)(appDelegate, @selector(setShouldReinitializeAudio:), NO);
            }
        }
