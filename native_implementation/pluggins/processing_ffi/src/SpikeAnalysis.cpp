    ...
    namespace analysis {
        ...
        SpikeAnalysis::findSpikes(const char *filePath, short **outValuesPos, int **outIndicesPos, float **outTimesPos,
            ...
            while ((read = drwav_read_s16(wavPtr, bufferSize, samples)) > 0) {
                ...
                backyardbrains::utils::SignalUtils::deinterleaveSignal(deinterleavedSamples, samples, sampleCount,
                                                                       channelCount);
                for (int i = 0; i < channelCount; i++) {
            ...
            //__android_log_print(ANDROID_LOG_DEBUG, TAG, "%ld - AFTER FINDING DEVIATIONS",static_cast<long>(currentTimeInMilliseconds() - start));

            // 2. SORT DEVIATIONS ASCENDING
            for (int i = 0; i < channelCount; i++) {
                std::sort(standardDeviations[i], standardDeviations[i] + deviationCounters[i], std::greater<>());
            }
            //__android_log_print(ANDROID_LOG_DEBUG, TAG, "%ld - AFTER SORTING AND REVERSING DEVIATIONS",static_cast<long>(currentTimeInMilliseconds() - start));
            ...
            delete[] currentTime;
            delete[] currentIndex;
            //__android_log_print(ANDROID_LOG_DEBUG, TAG, "%ld - AFTER FINDING SPIKES",static_cast<long>(currentTimeInMilliseconds() - start));
            for (int channel = 0; channel < channelCount; channel++) {
            ...
            //__android_log_print(ANDROID_LOG_DEBUG, TAG, "%ld - AFTER FILTERING SPIKES",static_cast<long>(currentTimeInMilliseconds() - start));
            //for (int channel = 0; channel < channelCount; channel++) {
                //__android_log_print(ANDROID_LOG_DEBUG, TAG, "FOUND POSITIVE: %d",(spikeCounter[channel] - removedCounter[channel]));
                //__android_log_print(ANDROID_LOG_DEBUG, TAG, "FOUND NEGATIVE: %d",(spikeNegCounter[channel] - removedNegCounter[channel]));
            //}
