...
FFI_PLUGIN_EXPORT int get_nwb_file_data(uint8_t* buffer, uint32_t* buffer_size, uint32_t* buffer_size_raw, int start_size, int end_size) {
    ...
    if (buffer == nullptr || end_size < static_cast<int>(g_nwb_file_data.size())) {
        return -2; // Invalid buffer or buffer too small
    }

    // std::copy(g_nwb_file_data.begin(), g_nwb_file_data.end(), buffer);
    std::copy(start_size, end_size, buffer);
    buffer_size[0] = end_size - start_size;
    buffer_size_raw[0] = g_nwb_file_data.size();
    return static_cast<int>(g_nwb_file_data.size());
}
