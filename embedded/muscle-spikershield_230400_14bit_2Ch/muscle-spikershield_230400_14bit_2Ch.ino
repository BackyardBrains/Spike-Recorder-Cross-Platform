...
void setup()
{
    ...
    OCR1A = interrupt_Number * 2;                             //output compare registers 
    TCCR1B |= (1 << WGM12);                               //turn on CTC mode
    TCCR1B |= (1 << CS11);                                //set CS11 bit for 8 prescaler
    TIMSK1 |= (1 << OCIE1A);                              //enable timer compare interrupt

    numberOfChannels = 2;                                 //set initial number of channels
  ...
  //do the auxiliary computation once per timer interrupt
   if(readyToDoAuxComputation==1)
   {
        ...
        readyToDoAuxComputation = 0;
                  ...
                  outputFrameBuffer[0]= (test1>>7)| 0x80;           //convert data to frame according to protocol
                  outputFrameBuffer[1]=  test1 & 0x7F;              //first bit of every byte is used to flag start of the frame
                 ...
                 // outputFrameBuffer[1]=  envelopeFirstChanel & 0x7F;
                  ...
                  outputFrameBuffer[2]= (test2>>7)& 0x7F;           //so first bit is set only on first byte of frame (| 0x80)
                  outputFrameBuffer[3]=  test2 & 0x7F;
